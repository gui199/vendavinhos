...
def get_dados(site):
    ...
    req = requests.get(site, timeout=15)
...
def get_lista_orden_cli():
    ...
    for client_maior_valor in range(len(dframe_cli)):
        valor = round(
            dframe_hc[dframe_hc['cliente'] == \
                      "0" + str(dframe_cli.iloc[client_maior_valor]["cpf"]).replace(
                          "-", ".")]['valorTotal'].sum(), 2)
...
def get_maior_compra(ano):
    ...
    dframe_hc = get_dados(SITE_HC)
    # corrigir o cpf errado do cliente
    dframe_hc.loc[dframe_hc['cliente'] == '000.000.000.01', 'cliente'] = \
    "0000.000.000.01"
...
def home():
    ...
    return '''<h1>Home</h1>
    <p><a href="/api/v1/recursos/clientes/all" rel="gallery"> 
    1 - Liste os clientes ordenados pelo maior valor total em compras</a></p>
    <p><a href="/api/v1/recursos/clientes/maiorcompra/2016" rel="gallery"> 2 
    - Mostre o cliente com maior compra única no último ano (2016).</a></p>'''
...
def api_all():
    # variavel que armazena a lista/tabela com os clientes ordenados pelo
    # maior valor total em compras
